...
```bash
# from server/, with the venv active and DATABASE_URL etc. already set
0 8 * * * cd /app/server && python -m scripts.send_digest
```

Point `DIGEST_RECIPIENT_EMAILS` at the HR admin distribution list and `SMTP_*` at a real (or relay) mail account to make it actually send; otherwise it stays a no-op that only writes to the app's structured log.
